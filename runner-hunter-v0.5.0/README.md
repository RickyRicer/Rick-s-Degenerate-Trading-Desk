...
## v0.5.0 — Multi-source on-chain intelligence

Robinhood Chain security no longer treats GoPlus as the authority. v0.5.0 merges independent evidence from:
- Robinhood Chain JSON-RPC: bytecode, `owner()`, `paused()`, `totalSupply()` probes
- Blockscout PRO: source verification, proxy/deployer provenance, holder list/counters
- GoPlus: independent honeypot/tax/control signals when available

Holder concentration is now calculated independently from Blockscout when total supply is available. Missing honeypot/tax/sellability evidence remains UNKNOWN; source verification alone never creates a PASS. This release intentionally does not claim true linked-wallet bundle detection yet—funding-graph/same-block clustering is the next stage.
