...
const immutableReportCache = new Map<string, { report: BlockscoutContractReport; cachedAt: number }>();

const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

function retryableBlockscoutError(error: unknown): boolean {
  const text = describeError(error);
  return /HTTP (429|5\d\d)|fetch failed|ECONNRESET|ETIMEDOUT|UND_ERR|socket/i.test(text);
}
...
async function getJsonOnce(url: string): Promise<any> {
  const headers = {
...
async function getJson(url: string): Promise<any> {
  ...
  for (let attempt = 0; attempt < delays.length; attempt++) {
    ...
    try {
      ...
      lastError = error;
      if (!retryableBlockscoutError(error) || attempt === delays.length - 1) throw error;
      console.warn(`[Blockscout] transient error; retrying ${attempt + 2}/${delays.length}: ${describeError(error)}`);
    }
  }
  throw lastError;
}
...
export async function getRobinhoodBlockscoutReport(token: TokenSnapshot): Promise<BlockscoutContractReport> {
  ...
  try {
    ...
    immutableReportCache.set(token.tokenAddress.toLowerCase(), { report, cachedAt: Date.now() });
    ...
    const cached = immutableReportCache.get(token.tokenAddress.toLowerCase());
    if (cached) {
      const ageMinutes = Math.max(0, Math.round((Date.now() - cached.cachedAt) / 60000));
      const fallback: BlockscoutContractReport = {
        ...cached.report,
        notes: [
          ...cached.report.notes,
          `Blockscout currently degraded; reusing cached immutable contract evidence from ${ageMinutes}m ago`,
          message
        ]
      };
      console.warn(`[Blockscout] robinhood:${token.symbol} ${message}; using cached immutable evidence age=${ageMinutes}m`);
      return fallback;
    }
    console.warn(`[Blockscout] robinhood:${token.symbol} ${message}`);
    return { checked: false, notes: [message] };
