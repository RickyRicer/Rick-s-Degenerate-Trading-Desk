...
function evidenceIcon(status: "PASS" | "WARN" | "FAIL" | "UNKNOWN") {
  return status === "PASS" ? "✅" : status === "WARN" ? "⚠️" : status === "FAIL" ? "🛑" : "❔";
}
...
export function compactTokenCard(t: TokenSnapshot, a: TradeIntent, riskFlags: string[] = [], rank?: number, security?: SecurityReport) {
  ...
  const lines = [
    ...
    ...(security?.deployerIsKnownInfrastructure ? [`🏭 Launchpad: ${html(security.deployerInfrastructureName ?? security.deployerName ?? "known infrastructure")} • known infrastructure`] : security?.deployerScanCount && security.deployerScanCount > 1 ? [`🏭 Same deployer: ${security.deployerScanCount} scan tokens${security.deployerName ? ` • ${html(security.deployerName)}` : ""}`] : []),
...
export function tokenCard(t: TokenSnapshot, a?: TradeIntent, riskFlags: string[] = [], rank?: number, security?: SecurityReport) {
  ...
  if (security?.checked) {
    ...
    if (security.implementationName) facts.push(`Impl: ${security.implementationName}`);
    else if (security.implementationAddress) facts.push("Impl: resolved");
    if (security.abiFunctionNames?.length) facts.push(`ABI funcs: ${security.abiFunctionNames.length}`);
    if (facts.length) lines.push(facts.join(" | "));
    if (security.evidence?.length) {
      const prioritized = [...security.evidence].sort((a, b) => {
        const rank = { FAIL: 0, UNKNOWN: 1, WARN: 2, PASS: 3 } as const;
        return rank[a.status] - rank[b.status];
      }).slice(0, 12);
      lines.push("", "🧪 Evidence matrix:", ...prioritized.map(e => `${evidenceIcon(e.status)} ${html(e.label)} — ${html(e.detail)} <i>(${html(e.source)})</i>`));
    }
  ...
  if (security?.creatorAddress) {
    ...
    if (security.deployerIsKnownInfrastructure) {
      lines.push(`✅ Known infrastructure: ${html(security.deployerInfrastructureName ?? security.deployerName ?? "recognized deployer")}${security.deployerInfrastructureKind ? ` • ${html(security.deployerInfrastructureKind)}` : ""}`);
      if (security.deployerScanCount && security.deployerScanCount > 1) lines.push(`Launched ${security.deployerScanCount} candidates in this scan (expected for shared launch infrastructure)`);
    } else if (security.deployerScanCount && security.deployerScanCount > 1) {
      lines.push(`Seen on ${security.deployerScanCount} scan candidates: ${(security.relatedDeployments ?? []).map(html).join(" • ")}`);
    }
