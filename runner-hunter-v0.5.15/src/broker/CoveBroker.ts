...
const read = new CoveMcpClient({ access: "read", clientVersion: "0.5.10" });
const write = new CoveMcpClient({ access: "paper-write", clientVersion: "0.5.10" });
...
function chainValue(token: TokenSnapshot, numeric: boolean) {
  const ids: Record<string, number> = { robinhood: 4663, bsc: 56, ethereum: 1, base: 8453, arbitrum: 42161 };
  if (numeric) return ids[token.chainId] ?? token.chainId;
...
function buildToolArgs(tool: McpTool, context: {
  ...
  for (const [key, prop] of Object.entries<any>(props)) {
    ...
    if (context.token && ["chainid","networkid"].includes(normalized)) { args[key] = chainValue(context.token, true); continue; }
...
export class CoveBroker implements Broker {
  ...
  private async marketOrder(side: "BUY" | "SELL", token: TokenSnapshot, usdAmount: number): Promise<BrokerTradeResult> {
    ...
    const idempotencyKey = `runner-hunter-paper-${side.toLowerCase()}-${token.chainId}-${token.tokenAddress}-${randomUUID()}`;
    const args = buildToolArgs(tool, { accountId, token, usdAmount, idempotencyKey });
    const raw = extractMcpContent(await write.callPaperWriteTool(tool.name, args));
