...
export const config = {
  ...
  authorizedTelegramUserId: process.env.AUTHORIZED_TELEGRAM_USER_ID
    ...
    : undefined,
  brokerMode: (process.env.BROKER_MODE ?? "mock").toLowerCase(),
  coveReadToken: process.env.COVE_READ_TOKEN ?? "",
  coveWriteToken: process.env.COVE_WRITE_TOKEN ?? "",
  coveMcpUrl: process.env.COVE_MCP_URL ?? "https://production.cove.trade/api/mcp",
  coveMcpDebug: bool("COVE_MCP_DEBUG", false),
  ...
  // Keep the future/live floor conservative, but use a lower floor for paper execution testing.
  minLiquidityUsd: num("MIN_LIQUIDITY_USD", 50000),
  paperMinLiquidityUsd: num("PAPER_MIN_LIQUIDITY_USD", 30000),
  paperLiquidityGraceFloorUsd: num("PAPER_LIQUIDITY_GRACE_FLOOR_USD", 25000),
  paperEligibilityWindowMinutes: num("PAPER_ELIGIBILITY_WINDOW_MINUTES", 5),
